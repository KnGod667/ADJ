...
func _init():
	...
	print("draw terrain initialized")
...
func initialize_render(framebuffer_format : int):
	...
	p_shader = compile_shader(custom_source_vertex, custom_source_fragment)
	p_wire_shader = compile_shader(custom_source_vertex, source_wire_fragment)
...
const custom_source_vertex = "
	...
	// This is the uniform buffer that contains all of the settings we sent over from the cpu in _render_callback. Must match with the one in the fragment shader.
		...
		void main(){
			float x_sine = sin(a_Position.x);
			float y_sine = sin(a_Position.y);
			pos = a_Position + vec3(0.0,0.0,x_sine+y_sine);
			v_Color=a_Color;
		}
"
